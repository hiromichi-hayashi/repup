...
`Skill` ツールで `spec-driven-dev <issue-id>` を invoke。Phase 0-4 (Linear取得・設計書読込・整合チェック・ブレスト・計画・実装) を駆動する。
