...
> 通知は **iOS プッシュ(Expo Push)のみ**(対象外は [01](01_concept.html) §7.3)。
...
| N-03 | 連続途切れ警告(任意) | 連続が途切れそう/途切れた | F04 §6.4 |

> N-03 は任意。最小は N-01・N-02(リマインド)。
...
```
[cron] 定期実行(例: 5〜15分間隔)
   │  対象抽出: Schedule(予定日)+ User設定 + PushToken
   ▼
...
- `data.type` と `scheduleId` で、タップ時にホーム(SC-01)や記録(SC-05)へディープリンク

---

## 7. 権限・失敗時

- 通知許可はオンボーディング(SC-02)で取得。未許可なら設定(SC-09)から再取得導線
