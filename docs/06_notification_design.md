...
> MVP の通知は **iOS プッシュ(Expo Push)のみ**。LINE / Google カレンダー連携はスコープ外([01](01_concept.html) §7.3)。
...
| N-04 | 連続途切れ警告(任意) | 連続が途切れそう/途切れた | F04/F05 |
...
1. **当日リマインド(N-01)**: 今日が `Schedule.date`、または繰り返しルールに今日の曜日が含まれ `active=true` の予定。配信予定時刻に達したもの
2. **前日リマインド(N-02)**: 明日が対象日の予定。前日配信時刻に達したもの
